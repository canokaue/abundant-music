...
class LoadSamplesAsyncOperation extends AsyncOperation{
    constructor(options) {
        if (options) {
...
class LoadSM2SoundsAsyncOperation extends LoadSamplesAsyncOperation{
    constructor(options) {
        super(options);
    }
    start() {
        const that = this;
        if (!LoadSM2SoundsAsyncOperation.sm2Loaded) {
                    ...
                    if (textStatus == "success") {
                        ...
                        soundManager.setup({
                            ...
                            onready: function () {
                                ...
                                LoadSM2SoundsAsyncOperation.sm2Loaded = true;
                                that.start();
                            }
                        });
                    }
        ...
        function addBuffer(buffer, url, index) {
            LoadSM2SoundsAsyncOperation._bufferMap[url] = buffer;
        ...
        function loadBuffer(index) {
            ...
            const theBuffer = LoadSM2SoundsAsyncOperation._bufferMap[url];
            if (theBuffer) {
...
class LoadAudioBuffersAsyncOperation extends LoadSamplesAsyncOperation {
    ...
    start() {
        ...
        function addBuffer(buffer, url, index) {
            LoadAudioBuffersAsyncOperation._bufferMap[url] = buffer;
            that.resultBuffers[index] = buffer;
            that.loadedCount++;
            const fractionDone = that.loadedCount / that.bufferUrls.length;
            const intProgress = Math.round(100 * fractionDone);
        ...
        function loadBuffer(index) {
            const url = that.bufferUrls[index];
            const theBuffer = LoadAudioBuffersAsyncOperation._bufferMap[url];
            if (theBuffer) {
...
class AsyncWorkerTask extends AsyncOperation{
    constructor(options) {
        super(options);
        this.requireLogin = false;
