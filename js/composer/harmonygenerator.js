...
class HarmonyGenerator extends DfsSolver{
    ...
    calculateSeventhToTriadCosts(node, possibleNextStates, possibleNextStateLikelihoods, possibleNextStateCosts) {
    ...
    calculateSusCosts(node, possibleNextStates, possibleNextStateLikelihoods, possibleNextStateCosts, sus2Likelihood, sus4Likelihood) {
        const state = node.state;
        const fromHarmony = state.harmony;
        for (let i = 0; i < possibleNextStates.length; i++) {
            const toHarmony = possibleNextStates[i].harmony;
    ...
    calculateBeatStrengthRepetitionCosts(node, possibleNextStates, possibleNextStateLikelihoods, possibleNextStateCosts) {
    ...
    getChordsStuff(index, state, likelihood, cost, seventhLikelihoodArr, triadLikelihoodArr, seventhCostArr, triadCostArr, resultStates, resultLikelihoods, resultCosts) {
    ...
    getBassNeighbourChords(fromHarmony, possibleRoots, possibleInversions, possibleSusRoots, possibleMixtureRoots) {
        const harmonies = [];
