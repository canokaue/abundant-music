...
class SplitZoneCollection {
    ...
    splitAndCopy(minLengthTicks, bestSplitIndex, current, velocityMultipliers, notes, numerator, denominator) {
    ...
    split(splitStrategy, dottedSplitStrategy, tripletSplitStrategy, minLengthTicks, bestSplitIndex, input, velocityMultipliers, dottedVelocityMultipliers, tripletVelocityMultipliers, numerator, denominator) {
        ...
        return this.splitAndCopy(minLengthTicks, bestSplitIndex, input, theVelocityMultipliers, newNotes, numerator, denominator);
    }
    getSplitBeat(module, start, noteCount, density, numerator, denominator) {
        ...
        while (true) {
            const next = this.singleSplit(module, density, currentBeat, noteCount, numerator, denominator, iteration, applicationMap);
            iteration++;
    ...
    singleSplit(module, density, input, beatIntensity, numerator, denominator, iteration, applicationMap) {
        let result = null;
        ...
        if (applicable.length == 0) {
            ...
            if (applicable.length > 0) {
                ...
                if (applicable.length > 1) {
                    ...
                    const index = sampleIndexIntegerDistribution(this.rnd, getProbabilityDistribution(likelihoods));
                ...
                // Use default strategy
                result = this.split(this.defaultSplitStrategy, this.defaultDottedSplitStrategy, this.defaultTripletSplitStrategy, minLengthTicks, bestSplitIndex, input, this.defaultVelocityMultipliers, numerator, denominator);
            }
        }
        if (result == null && this.tryHalveIfStrategyFails) {
            result = this.split(SplitStrategy.HALVE, DottedSplitStrategy.TWO_DOTTED, TripletSplitStrategy.HALVE, minLengthTicks, bestSplitIndex, input, this.defaultVelocityMultipliers, this.defaultDottedVelocityMultipliers, this.defaultTripletVelocityMultipliers, numerator, denominator);
        }
        return result;
    }
}
