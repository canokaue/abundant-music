class MotifRenderElement extends PositionedRenderElement {
    ...
    render(harmony, elements, theVoiceLine, noteAbsoluteNotes, state) {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            const beatLength = e.getBeatLength(che.tsNumerator, che.tsDenominator);
            if (!e.rest) {
                    ...
                    if (noteLength > 0.0) {
                        ...
                        if (e.fillers) {
                            ...
                            for (let j = 0; j < e.fillers.length; j++) {
                                ...
                                const fillerPosition = currentTime + state.sectionTime + positionUnitToBeats(filler.positionOffset, filler.positionOffsetUnit, che.tsNumerator, che.tsDenominator);
                                let fillerLength = positionUnitToBeats(filler.length, filler.lengthUnit, che.tsNumerator, che.tsDenominator);
    ...
    figurate(harmony, elements, noteAbsoluteNotes, voiceHarmonyElements, elementHarmonyIndices, previousVoiceElements, nextVoiceElements, theVoiceLine, theMotif, module, section) {
    ...
    assignHorizontalRelativeMotifElements(harmony, elementHarmonyIndices, theVoiceLine, voiceElements, noteAbsoluteNotes) {
    ...
    assignVerticalRelativeMotifElements(voiceElements, voiceHarmonyElements, voiceVoiceLineElements, noteAbsoluteNotes) {
        for (let i = 0; i < voiceElements.length; i++) {
            const vnme = voiceElements[i];
    ...
    gatherVoiceAndHarmonyInfo(harmony, elements, theVoiceLine, elementHarmonyIndices, voiceHarmonyElements, voiceVoiceLineElements, voiceElements, previousVoiceElements, nextVoiceElements) {
        const he = harmony.get(0);
        const startBeatTime = positionUnitToBeats(this.startTime, this.startTimeUnit, he.tsNumerator, he.tsDenominator, harmony);
        let currentTime = startBeatTime;
    ...
    getOrCreateVoiceLine(state, harmony) {
        let theVoiceLine = getObjectWithId(this.voiceLine, state.plannedVoiceLines);
        if (this.useVoiceLine) {
            ...
            for (let i = 0; i < harmony.getCount(); i++) {
                ...
                const offset = getItemFromArrayWithStartEndItems(0, this.offsets, harmony.getCount(), i, this.startOffsets, this.endOffsets);
    ...
    renderBatch(state) {
        ...
        this.gatherVoiceAndHarmonyInfo(harmony, elements, theVoiceLine, elementHarmonyIndices, voiceHarmonyElements, voiceVoiceLineElements, voiceElements, previousVoiceElements, nextVoiceElements);
        ...
        this.figurate(harmony, elements, noteAbsoluteNotes, voiceHarmonyElements, elementHarmonyIndices, previousVoiceElements, nextVoiceElements, theVoiceLine, theMotif, state.module, state.section);
        // Render all
