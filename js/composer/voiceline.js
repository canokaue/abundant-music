...
class SimpleBassVoiceLine extends VoiceLine {
    ...
    getSingleStepVoiceLineElements(harmony, module) {
        ...
        for (let i = 0; i < harmony.getCount(); i++) {
            ...
            const octave = getItemWithDefaultWithStartEndItems(this.octaves, harmony.getCount(), i, this.startOctaves, this.endOctaves);
            absNote += octave * 12;
    ...
    getSingleStepVoiceLineElements(harmony, module) {
        ...
        if (useHintCurveLengthFractionAmplitudeMultiplier) {
            ...
            if (referenceFraction < 1) {
                ...
                // (1 - 1 + 2f)x -2f + 1
                // 2fx - 2f + 1
                const ampMultiplier = Math.max(0, Math.min(1, 2 * this.hintCurveLengthFractionAmplitudeMultiplier * referenceFraction -
        ...
        for (let i = 0; i < harmonyLength; i++) {
            if (this.isUndefined) {
                ...
                if (theCurve) {
                    ...
                    theHintIndex = getItemFromArrayWithStartEndItems(theHintIndex, hintIndices, harmonyLength, i, startHintIndices, endHintIndices);
                }
                const maxHintDistance = getItemFromArrayWithStartEndItems(6, this.maxHintDistances, harmonyLength, i, this.startMaxHintDistances, this.endMaxHintDistances);
                const penaltyMaxHintDistance = getItemFromArrayWithStartEndItems(3, this.penaltyMaxHintDistances, harmonyLength, i, this.startPenaltyMaxHintDistances, this.endPenaltyMaxHintDistances);
                const theChordBassConstraint = getItemFromArrayWithStartEndItems([], chordBassPitchClassConstraints, harmonyLength, i, startChordBassPitchClassConstraints, endChordBassPitchClassConstraints);
