...
class ClassicalVoiceLineGenerator extends VoiceLineGenerator {
    constructor(options) {
        super(options);
        this.voiceCount = getValueOrDefault(options, "voiceCount", 4);
        this.maxDomainSize = getValueOrDefault(options, "maxDomainSize", 100);
    ...
    getTwoStepCost(harmonyIndex, prevPrevStateIndex, prevStateIndex, stateIndex, verbose) {
        ...
        for (let i = 0; i < absoluteNotes.length; i++) {
            ...
            const count = this.getlargeLeapReverseDirectionPenaltyCount(prevPrevAbsoluteNotes[i], prevAbsoluteNotes[i], absoluteNotes[i]);
    ...
    prepareBeforeSearch() {
        ...
        function gatherDomain(harmonyIndex, voiceIndex, previousAbsNote, previousScaleIndex, currentAbsTuple, currentScaleIndexTuple, resultAbsoluteNoteTuples, resultScaleIndexTuples) {
            ...
            for (const scaleIndex in possibleScaleIndices) {
                ...
                if (voiceIndex < voiceCount - 1) {
                    gatherDomain(harmonyIndex, voiceIndex + 1, absNote, scaleIndex, absTuple, scaleIndexTuple, resultAbsoluteNoteTuples, resultScaleIndexTuples);
                }
                else {
