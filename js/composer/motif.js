...
class Motif {
    ...
    toString(options) {
        let result = "{";
        result += "" + $.map(this.motifElements, (o, i) => {
        ...
        if (this.inheritedMotif) {
            ...
            if (theRythm) {
                ...
                for (let j = 0; j < noteRythmElements.length; j++) {
                    const rythmElement = noteRythmElements[j];
                ...
                for (let j = 0; j < noteRythmElements.length; j++) {
                    ...
                    if (rythmElement.rest || elementZones[j] == -1) {
                        ...
                        if (!appliedZones[zoneIndex]) {
                            const zone = this.motifZones[elementZones[j]];
            ...
            for (let i = 0; i < this.motifElements.length; i++) {
                ...
                const list = e.getConstantMotifElements(module, harmony, harmonyBeatOffset, visitedMotifs);
                addAll(result, list);
            }
        }
        // Apply the modifiers
...
class SimpleSequenceMotifElement extends MotifElement {
    ...
    getConstantMotifElements(module, harmony, harmonyBeatOffset, visitedMotifs) {
        ...
        while (currentPosition < totalLength) {
            ...
            let beatLength = positionUnitToBeats(elementLength, this.elementLengthPatternUnit, harmonyElement.tsNumerator, harmonyElement.tsDenominator, harmony);
            let rest = false;
            ...
            if (this.verticalOffsetPattern.length > 0) {
                const realOffsetIndex = IndexBorderMode.getIndex(this.verticalOffsetPatternBorderMode, this.verticalOffsetPattern.length, index);
