    ...
    getPercussionMotifElements(module, noteRythmElements, harmony, harmonyBeatOffset) {
        ...
        for (let i = 0; i < noteRythmElements.length; i++) {
            ...
            if (!missingBeatCondition) {
                ok = false;
                const beatDivisor = positionUnitToBeats(this.beatConditionDivisorCheck, this.beatConditionDivisorCheckUnit, he.tsNumerator, he.tsDenominator, harmony);
        ...
        for (let i = 0; i < activeElements.length; i++) {
            const noteIndices = getItemFromArrayWithStartEndItems([], this.noteIndexPattern, activeElements.length, i, this.startNoteIndexPattern, this.endNoteIndexPattern);
            let positionOffsets = getItemFromArrayWithStartEndItems([], this.positionOffsetPattern, activeElements.length, i, this.startPositionOffsetPattern, this.endPositionOffsetPattern);
